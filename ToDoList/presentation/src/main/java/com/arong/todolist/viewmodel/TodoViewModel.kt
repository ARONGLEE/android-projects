...
import com.arong.data.repository.TodoRepositoryImpl
...
class TodoViewModel(application: Application) : AndroidViewModel(application) {
    private val mTodoRepository: TodoRepositoryImpl
    private val mTodoItems: LiveData<MutableList<Todo>>

    init {
        mTodoRepository = TodoRepositoryImpl(application)
        mTodoItems = mTodoRepository.list()
    }
//    fun getOne(id: Long): TODO { return mTodoRepository.getTodo(id) }
    fun getOne(id: Long) = mTodoRepository.getTodo(id)
//    fun getOne(id: Long) {
//        mTodoRepository.getTodo(id)
//    }
    // -> = 과 {} 차이가 있다.. 차이가 뭐지?? {}했을땐 액티비티에서 체크값 토글에서 뷰모델을 통해 isChecked 접근이 안됨.. => 리턴을 해주냐 안해주냐에 차이..ㅠㅠㅠㅠㅠ
